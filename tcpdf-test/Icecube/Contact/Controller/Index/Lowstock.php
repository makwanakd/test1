...
class Lowstock extends \Magento\Newsletter\Controller\Subscriber\NewAction
{
	...
	public function execute()
    {
        ...
        if (!$post) {
            ...
            if(isset($post['lspopupemail'])){
					//$email = 'test8.capital@gmail.com';
					$email = $post['lspopupemail'];
					
					$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
					$region = $objectManager->create('Magento\Directory\Model\Region')->load($post['region_id_pennsylvania_lowstock']);
					$regionName = '';
                    if ($region) {
                        $regionName = $region->getName();
                    }
                    $telephone = $post['lspopupphone'];
					$finalNumber = '';
		            ...
		            $mergeVars = array("FNAME" => $post['lspopupfname'], "LNAME" => $post['lspopuplname'], "STATE" => $regionName, "PHONE" => $finalNumber, "CITY" => $post['lspopupcity']);
		            ...
		            try {
		            	...
		            	$return = $api->lists->members->addOrUpdate('48464bb633', $emailHash, null, $status, $mergeVars, null, null, null, null, $email, $status);
